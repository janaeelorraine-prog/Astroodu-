    ...
    function Editor() {
      ...
      function blankDraft(side) {
        return { name: "", known: "", dates: "", place: "", rel: "", life: "", status: "unknown",
                 gap: false, photo: "", currentPhoto: "", removePhoto: false, side: side || "pat",
                 facts: [], stories: [], flags: [], sources: [] };
      }
      ...
      function openEdit(branch, gen, person) {
        ...
        setForm({ mode: "edit", title: "Edit " + (m.name || person.name), branchId: branch.id, genIndex: gen.genIndex, personId: person.id,
          draft: { name: m.name || "", known: m.known || "", dates: m.dates || "", place: m.place || "", rel: m.rel || "",
                   ...
                   gap: !!m.gap, photo: "", currentPhoto: currentPhotoOf(person.id),
                   removePhoto: false, side: m.side || branch.side,
                   facts: (m.facts || []).map(function (f) { return { k: (f && f[0]) || "", v: (f && f[1]) || "" }; }),
                   stories: (m.stories || []).map(function (s) { return { kind: s.kind === "record" ? "record" : "family", text: s.text || "" }; }),
                   flags: (m.flags || []).map(function (t) { return { text: typeof t === "string" ? t : (t && t.text) || "" }; }),
                   sources: (m.sources || []).map(function (s) { return { label: s.label || "", url: s.url || "" }; }) } });
      }
      ...
      function saveForm() {
        ...
        // full, editable content — the person's whole write-up in their words
        obj.facts = (d.facts || []).map(function (f) { return [String(f.k || "").trim(), String(f.v || "").trim()]; })
          .filter(function (f) { return f[0] || f[1]; });
        obj.flags = (d.flags || []).map(function (f) { return String(f.text || "").trim(); }).filter(Boolean);
        obj.stories = (d.stories || []).map(function (s) { return { kind: s.kind === "record" ? "record" : "family", text: String(s.text || "").trim() }; })
          .filter(function (s) { return s.text; });
        obj.sources = (d.sources || []).map(function (s) { return { label: String(s.label || "").trim(), url: String(s.url || "").trim() }; })
      ...
      function personForm() {
        ...
        // an editable list of rows (facts, stories, flags, sources)
        function listBlock(title, key, fields, addLabel, hint) {
          var arr = d[key] || [];
          function update(na) { var nf = Object.assign({}, form); nf.draft = Object.assign({}, d); nf.draft[key] = na; setForm(nf); }
          function setCell(idx, fk, val) { update(arr.map(function (rw, i) { if (i !== idx) return rw; var c = Object.assign({}, rw); c[fk] = val; return c; })); }
          function removeRow(idx) { update(arr.filter(function (_, i) { return i !== idx; })); }
          function addRow() { var e = {}; fields.forEach(function (fl) { e[fl.k] = fl.def || ""; }); update(arr.concat([e])); }
          return h("div", { style: { margin: "6px 0 14px" } },
            h("div", { style: lbl() }, title),
            hint ? h("div", { style: { fontSize: "11.5px", color: MUTE, marginBottom: "6px" } }, hint) : null,
            arr.map(function (rw, idx) {
              return h("div", { key: idx, style: { display: "flex", gap: "6px", alignItems: "flex-start", marginBottom: "6px" } },
                fields.map(function (fl) {
                  var val = rw[fl.k] || "";
                  if (fl.select) return h("select", { key: fl.k, value: val || fl.def, onChange: function (e) { setCell(idx, fl.k, e.target.value); }, style: Object.assign({}, input(false), { flex: fl.flex || "0 0 140px" }) },
                    fl.select.map(function (o) { return h("option", { key: o[0], value: o[0] }, o[1]); }));
                  if (fl.area) return h("textarea", { key: fl.k, value: val, rows: fl.rows || 2, placeholder: fl.ph || "", onChange: function (e) { setCell(idx, fl.k, e.target.value); }, style: Object.assign({}, input(true), { flex: fl.flex || 1 }) });
                  return h("input", { key: fl.k, value: val, placeholder: fl.ph || "", onChange: function (e) { setCell(idx, fl.k, e.target.value); }, style: Object.assign({}, input(false), { flex: fl.flex || 1 }) });
                }).concat([h("button", { key: "rm", title: "Remove", onClick: function () { removeRow(idx); }, style: Object.assign({}, miniBtn(RUST), { flex: "0 0 auto" }) }, "×")]));
            }),
            h("button", { onClick: addRow, style: miniBtn() }, addLabel || "+ Add"));
        }
        function pickPhoto() {
          ...
          h("div", { style: formSheet() },
            ...
            h("div", { style: { fontSize: "12.5px", color: MUTE, marginBottom: "16px" } },
              form.mode === "child" ? "They'll be added in the generation below."
                : form.mode === "edit" ? "Everything here is editable — rewrite any of it in your own voice. Photos and gallery images are kept."
            ...
            field("Life / story (optional)", "life", { area: true, rows: 6, ph: "Their story, in your words — where they lived, their work, family, who they were." }),
            listBlock("Facts (optional)", "facts",
              [{ k: "k", ph: "Label — e.g. Married", flex: "0 0 34%" }, { k: "v", ph: "Detail", flex: 1 }],
              "+ Add a fact", "Short label-and-detail pairs shown on the card."),
            listBlock("Stories & memories (optional)", "stories",
              [{ k: "kind", select: [["family", "Family memory"], ["record", "From a record"]], def: "family", flex: "0 0 150px" }, { k: "text", ph: "The memory, in your words", area: true, flex: 1 }],
              "+ Add a memory"),
            listBlock("Open questions (optional)", "flags",
              [{ k: "text", ph: "Something still uncertain about this person", area: true, flex: 1 }],
              "+ Add an open question", "Shown as “open questions” on the card."),
            listBlock("Sources (optional)", "sources",
              [{ k: "label", ph: "Label", flex: 1 }, { k: "url", ph: "https://…", flex: 1 }],
              "+ Add a source"),
